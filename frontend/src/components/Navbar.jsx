...
function Navbar() {
  ...
  return (
    <Box bg="brand.700" px={7} h="60px" position="sticky" top={0} zIndex={100}
      ...
      <Flex h="100%" align="center" justify="space-between">
        ...
        <HStack spacing={1} display={{ base: 'none', md: 'flex' }}>
          {navLinks.map((link) => {
            ...
            return (
              <ChakraLink
                as={Link} to={link.path} key={link.path}
                px={4} py={2} borderRadius="md" fontSize="sm"
        ...
        <Menu>
          <MenuButton as={Button} variant="ghost" px={2}
            ...
            <HStack spacing={2}>
              <Avatar size="sm" name={user?.name || 'Admin'} bg="blue.400" color="white" />
